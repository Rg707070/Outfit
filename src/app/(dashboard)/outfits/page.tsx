...
export default function OutfitsPage() {
  ...
  async function shareOutfit(outfit: Outfit) {
    ...
    alert(t.outfits.shareCopied)
  }

  async function deleteOutfit(outfit: Outfit) {
    if (!confirm(t.outfits.deleteConfirm(outfit.name))) return
  ...
  async function scheduleToday(outfit: Outfit) {
    ...
    await supabase.from('calendar_outfits').upsert({ user_id: user.id, outfit_id: outfit.id, date: today }, { onConflict: 'user_id,date' })
    alert(t.outfits.scheduledToday(outfit.name))
  }
      ...
      <div className="mt-8">
        {loading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {Array.from({ length: 6 }).map((_, i) => <div key={i} className="bg-gray-100 rounded-2xl h-64 animate-pulse" />)}
          </div>
        ) : outfits.length === 0 ? (
          <div className="text-center py-20">
            <span className="text-5xl">✨</span>
            <p className="text-gray-500 mt-4 text-lg font-medium">{t.outfits.noOutfits}</p>
            <p className="text-gray-400 text-sm mt-1">{t.outfits.noOutfitsSub}</p>
            <Link href="/outfits/new"><Button className="mt-6"><Plus size={16} />{t.outfits.createFirst}</Button></Link>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {outfits.map(outfit => (
              <OutfitCard key={outfit.id} outfit={outfit} onToggleFavorite={toggleFavorite} onShare={shareOutfit} onDelete={deleteOutfit} onScheduleToday={scheduleToday} />
            ))}
          </div>
        )}
      </div>
...
function OutfitCard({ outfit, onToggleFavorite, onShare, onDelete, onScheduleToday }: {
  outfit: Outfit
  ...
  return (
    <div className="group bg-white rounded-2xl border border-gray-100 overflow-hidden hover:shadow-md transition-shadow">
      <div className="h-48 bg-gradient-to-br from-gray-50 to-gray-100 relative flex items-center justify-center">
        {outfit.image_url ? <img src={outfit.image_url} alt={outfit.name} className="w-full h-full object-cover" /> : <span className="text-5xl">👔</span>}
        <div className="absolute top-3 right-3 flex gap-1.5">
          {outfit.is_favorite && <span className="text-red-500">❤️</span>}
          {outfit.is_public && <span className="text-xs bg-green-100 text-green-700 px-2 py-0.5 rounded-full font-medium">{t.outfits.public}</span>}
        </div>
        <div className="absolute top-3 left-3 flex gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
          <button onClick={() => onToggleFavorite(outfit)} className="p-1.5 bg-white rounded-full shadow-sm">
            <Heart size={14} className={outfit.is_favorite ? 'fill-red-500 text-red-500' : 'text-gray-400'} />
          </button>
          <button onClick={() => onShare(outfit)} className="p-1.5 bg-white rounded-full shadow-sm">
            <Share2 size={14} className="text-gray-400" />
          </button>
          <button onClick={() => onDelete(outfit)} className="p-1.5 bg-white rounded-full shadow-sm">
      ...
      <div className="p-4">
        ...
        <div className="flex items-center gap-2 mt-3">
          {outfit.occasion && <span className="text-xs bg-gray-100 text-gray-600 px-2.5 py-1 rounded-full">{outfit.occasion}</span>}
          {outfit.season && <span className="text-xs bg-gray-100 text-gray-600 px-2.5 py-1 rounded-full capitalize">{outfit.season}</span>}
        </div>
        <div className="flex gap-2 mt-4">
          <Button size="sm" variant="secondary" className="flex-1" onClick={() => onScheduleToday(outfit)}>
            <Calendar size={14} />{t.outfits.wearToday}
          </Button>
          <Button size="sm" variant="ghost" onClick={() => onShare(outfit)}><Share2 size={14} /></Button>
        </div>
      </div>
    </div>
  )
