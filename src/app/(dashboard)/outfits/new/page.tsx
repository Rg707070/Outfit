...
export default function NewOutfitPage() {
  ...
  async function handleSave() {
    ...
    const { data: outfit, error } = await supabase.from('outfits').insert({
      user_id: user.id, name, description: description || null, occasion: occasion || null,
      season: (season || null) as Season | null, is_public: isPublic,
      image_url: selectedItems.find(i => i.image_url)?.image_url ?? null,
    }).select().single()
    if (error || !outfit) { setSaving(false); return }
    await supabase.from('outfit_items').insert(selectedItems.map((item, idx) => ({ outfit_id: outfit.id, wardrobe_item_id: item.id, z_index: idx })))
    router.push('/outfits')
  }
    ...
    <div className="flex gap-6 h-[calc(100vh-4rem)]">
      <div className="flex-1 flex flex-col min-w-0">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900">{t.newOutfit.title}</h1>
            <p className="text-gray-500 text-sm">{t.newOutfit.sub}</p>
          </div>
        </div>
        ...
        <div className="flex gap-2 overflow-x-auto scrollbar-hide mb-4 pb-2">
          ...
          {CLOTHING_CATEGORIES.map(cat => (
            <button key={cat.value} onClick={() => setActiveCategory(cat.value as ClothingCategory)}
              ...
              <span>{cat.emoji}</span>{t.categories[cat.value as keyof typeof t.categories]}
            </button>
          ))}
        </div>
        ...
        <div className="flex-1 overflow-y-auto">
          {loading ? (
            ...
            <div className="text-center py-20">
              ...
              <p className="text-gray-500 mt-3">{t.newOutfit.noItemsInCat}</p>
              <Link href="/wardrobe"><Button className="mt-4" variant="secondary">{t.newOutfit.goToWardrobe}</Button></Link>
            </div>
          ) : (
            <div className="grid grid-cols-3 lg:grid-cols-4 gap-3">
              {filtered.map(item => {
                ...
                return (
                  <button key={item.id} onClick={() => toggleItem(item)}
                    className={`group relative aspect-square rounded-2xl overflow-hidden border-2 transition-all ${isSel ? 'border-black ring-2 ring-black/10' : 'border-transparent hover:border-gray-200'}`}>
                    <div className="w-full h-full bg-gray-50 flex items-center justify-center">
                      {item.image_url ? <img src={item.image_url} alt={item.name} className="w-full h-full object-cover" /> : <span className="text-3xl">{CLOTHING_CATEGORIES.find(c => c.value === item.category)?.emoji}</span>}
                    </div>
                    {isSel && <div className="absolute top-2 right-2 w-6 h-6 bg-black rounded-full flex items-center justify-center"><Check size={14} className="text-white" /></div>}
      ...
      <div className="w-80 flex-shrink-0 bg-white rounded-2xl border border-gray-100 flex flex-col">
        <div className="p-5 border-b border-gray-100">
          <h2 className="font-semibold text-gray-900 flex items-center gap-2"><Sparkles size={16} />{t.newOutfit.yourOutfit}</h2>
          <p className="text-xs text-gray-400 mt-0.5">{t.newOutfit.selected(selectedList.length)}</p>
        </div>

        <div className="p-5 flex-1 overflow-y-auto">
          {selectedList.length === 0 ? (
            <div className="h-40 border-2 border-dashed border-gray-200 rounded-2xl flex items-center justify-center">
              <p className="text-sm text-gray-400 text-center px-4">{t.newOutfit.selectPrompt}</p>
            </div>
          ) : (
            <div className="grid grid-cols-2 gap-2 mb-4">
              {selectedList.map(item => (
                <div key={item.id} className="relative group aspect-square rounded-xl overflow-hidden bg-gray-50 flex items-center justify-center">
                  {item.image_url ? <img src={item.image_url} alt={item.name} className="w-full h-full object-cover" /> : <span className="text-2xl">{CLOTHING_CATEGORIES.find(c => c.value === item.category)?.emoji}</span>}
                  <button onClick={() => toggleItem(item)} className="absolute top-1 right-1 w-5 h-5 bg-white rounded-full flex items-center justify-center shadow opacity-0 group-hover:opacity-100 transition-opacity">
          ...
          <div className="space-y-3">
            <Input placeholder={t.newOutfit.outfitName} value={name} onChange={e => setName(e.target.value)} />
            <textarea placeholder={t.newOutfit.description} value={description} onChange={e => setDescription(e.target.value)} rows={2}
            ...
            <Input placeholder={t.newOutfit.occasion} value={occasion} onChange={e => setOccasion(e.target.value)} />
            <select value={season} onChange={e => setSeason(e.target.value as Season)}
              ...
              {SEASONS.map(s => <option key={s.value} value={s.value}>{s.emoji} {t.seasons[s.value as keyof typeof t.seasons]}</option>)}
            </select>
        ...
        <div className="p-5 border-t border-gray-100">
          <Button onClick={handleSave} disabled={saving || !name || selectedList.length === 0} className="w-full" size="lg">
